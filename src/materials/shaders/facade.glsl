...
Facade shadeWall(vec2 uv, vec4 meta, vec3 tint, float flag, float night, float seedTime, vec3 viewTS) {
  ...
  if (style == 4.0) { winW = clamp(bayW * 0.38, 1.0, 1.8); winH = min(3.2, thisFloorH - 1.0); sill = 1.0; }
  ...
  bool hasWindow = !narrowWall && !aboveEave && bayHash > 0.06 && thisFloorH > 2.2;
  float win = hasWindow ? rectMask(vec2(bu, fv), wlo, whi, aa) : 0.0;
  ...
  bool hitGlass = inOpening && pq.x > wlo.x && pq.x < whi.x && pq.y > wlo.y && pq.y < whi.y;
  float revealSide = (inOpening && !hitGlass) ? 1.0 : 0.0;
  vec2 gq = hitGlass ? pq : vec2(bu, fv);
  float glass = hasWindow ? rectMask(gq, wlo + frameT, whi - frameT, aa) * (1.0 - revealSide) : 0.0;
  ...
  if (style == 0.0 || style == 4.0) {
    ...
    // Quoins / pilasters between bays on grand facades.
    if (style == 4.0) { float pil = 1.0 - smoothstep(0.18, 0.18 + aa, abs(bu - bayW * 0.5) - (bayW * 0.5 - 0.35)); color *= 1.0 + pil * 0.05; }
  }
  if (style == 1.0) {
...
// Mansard slope: zinc (default), natural slate or terracotta tile, keyed by the baked roof tint.
// uv = (metres along the eave, metres up the slope).
Facade shadeRoofSlope(vec2 uv, vec4 meta, vec3 tint, float seedTime) {
  Facade f;
  float seed = mod(meta.w, 256.0);
  float aa = max(fwidth(uv.x), fwidth(uv.y)) * 0.75 + 0.002;
  float u = uv.x, s = uv.y;
  ...
  bool slate = tint.b > tint.r + 0.02 && lum < 0.45;
  bool tile = tint.r > tint.b + 0.15;
  ...
  if (slate) {
    ...
    // Zinc: standing seams every 0.6 m, panel joints every 2 m, soft patina and a little grain waviness.
    vec3 zinc = vec3(0.50, 0.53, 0.57) * (0.92 + 0.08 * noise2(uv * 0.9 + seed));
    zinc *= 0.94 + 0.10 * noise2(uv * 0.23 + seed * 1.7);
    float seam = 1.0 - smoothstep(0.0, aa * 2.0, abs(fract(u / 0.6) - 0.5) * 0.6 - 0.02);
    color = zinc * (1.0 - seam * 0.18 * (1.0 - smoothstep(0.01, 0.06, aa)));
    ...
    n.x += seam * 0.35 * sign(fract(u / 0.6) - 0.5);
  }
  vec3 emissive = vec3(0.0);
