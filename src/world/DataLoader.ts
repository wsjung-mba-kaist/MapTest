...
/** Turn a PBM1 buffer into BufferGeometries (one per section). Empty sections are skipped. */
export function binMeshToGeometries(buf: ArrayBuffer): LoadedBinMesh {
  const { header, arrays } = parseBinMesh(buf);
  const sections = new Map<string, LoadedSection>();
  for (const s of header.sections) {
    if (!s.vertexCount || !s.indexCount) continue;
...
export async function loadBinMesh(url: string): Promise<LoadedBinMesh> {
  return binMeshToGeometries(await fetchBuffer(url));
}
