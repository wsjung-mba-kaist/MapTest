...
왼쪽 40 % 영역 아무 곳이나 누르면 그 자리에 조이스틱이 생기며(끝까지 밀면 달리기), 오른쪽을 드래그하면 시점이 돕니다. 오른쪽 아래 버튼 열:
비행 · 명소 · 지도 · ⋯(메뉴: 시간·날씨 · 시간대 순환 · 설정 · 링크 복사 · 스크린샷 · 도움말 · 정보). ▲▼(고도)는 비행 중에만 보이고,
승강기 안내 문구(`E · 1층으로`)를 탭하면 탑승합니다. 세로로 들면 명소 카드와 시간 패널이 버튼 열 위의 아래 시트로 열립니다.
모바일 프리셋(해상도 1x, AO·반사·나침반 끔, 그림자 2048, 파사드 디테일 320 m)이 자동 적용되고 설정에서 바꿀 수 있습니다. 첫 로딩은 청크 91 MB + 텍스처를 받으므로 Wi-Fi에서 1~2분 걸립니다.
...
| `L` | 명소 목록 (33곳, 가까운 순, 썸네일·방향 화살표·거리가 열린 동안 갱신) — 이름 검색(한·불·영), 분류 칩(기념물·박물관·다리…), `↑` `↓` `Enter`로 선택, 현재 위치 행 강조. 클릭하면 그곳으로 비행. 걷는 중이면 마우스 잠금이 풀리고 닫으면 복귀 |
| `I` | 명소 카드 펼치기/접기. 서 있는 곳이 아닌 명소의 카드에는 `여기로 이동` 버튼, 에펠탑 카드에는 승강기 안내. 걷다가 명소 반경에 들어가면 카드가 5초간 뜨고(같은 곳은 1분에 한 번) 칩은 늘 `현재 위치 · X` 또는 `가까운 명소 · X 320 m · 오른쪽 앞`을 보여줌. `Esc` 뒤에는 카드의 `W 위키백과` 링크를 클릭할 수 있음 |
...
| 시간 패널 추가 행 | 슬라이더 밑 일출·일몰 눈금 · `지금`(파리의 현재 날짜·시각) · `▶ ×60` / `▶ ×600` 시간 흐름 재생(슬라이더·프리셋·`N`·`,`·`.`를 건드리면 정지) · 날씨 버튼(맑음·흐림·비·안개 = `R`) · 계절 버튼(봄 4/20 · 여름 7/15 · 가을 10/20 · 겨울 1/15 · 오늘 = `?date=`). 시계 칩과 `P` 링크(`weather=`, `date=`)에 반영 |
| `,` / `.` | 15분 뒤로/앞으로 (`Shift`와 함께 1시간) — 태양 위치·창문 조명·가로등·탑 조명 연동 |
| `P` | 현재 위치·시점·시각을 담은 링크 복사 (`walk=1` 또는 `fly=1`) |
| `O` | 스크린샷 PNG 저장 |
| `M` | 미니맵 순환: 숨김 → 320 m → 800 m → 1.5 km → 숨김 (항공사진, 북쪽 위, 명소 점·이름·단축키 번호, 탑, 진행 방향). 걷는 중 마우스 휠로도 축척 순환, `Esc` 뒤 지도를 클릭하면 그 지점으로 비행 |
| 나침반 띠 | 화면 위 가운데 120° 호: 방위(N·E·S·W)와 15° 눈금이 시점을 따라 흐르고, 600 m 안 명소가 분류색 점(단축키 번호)으로, 가장 가까운 둘은 이름과 거리까지 표시. 서 있는 명소는 숨김. 설정에서 끔(폰 기본 꺼짐) |
...
| 터치 | 왼쪽 40 % 가상 조이스틱(끝까지 = 달리기) · 오른쪽 드래그 시점 · 버튼 열(비행 · 명소 · 지도 · ⋯ 메뉴; 비행 중 ▲▼; 승강기 문구 탭 = E); `pointer: coarse` 기기에서 자동, `?touch=1`로 강제. 모바일 프리셋(해상도 1x, AO·반사 끔). 명소 카드는 칩으로 시작하고 탭하면 펼쳐지며 세로 화면에서는 아래 시트 |
| `R` | 날씨 순환 (맑음 → 흐림 → 비 → 안개); 시간 패널의 날씨 버튼과 같음, `P` 링크에 `weather=`로 실림 |
| 설정 | 일시정지 메뉴(`Esc`)의 **설정**: 품질(자동·낮음·보통·높음) · 그림자 · 센강 반사 · AO · 해상도 배율 · 시점 감도 · 시야각 · 머리 흔들림 · 소리 크기 · 미니맵 · 나침반 · 명소 이름표 · 움직이는 도시 · 진단 정보. 바로 적용되고 이 브라우저(`localStorage` `paris.prefs`)에 저장됨. 품질이 **자동**이면 시작 뒤 몇 초의 프레임 시간을 재서 느리면(중앙값 40 ms 초과) 한 단계 낮추고 토스트로 알림(세션당 한 번, 올리지는 않음). URL 파라미터(`quality=`, `ao=0` 등)는 그 세션에서 저장된 설정보다 우선 |
...
UX: `minimap=1`(미니맵 켠 채 시작; 설정의 '미니맵 켜고 시작'과 같음) / `audio=0|debug` / `touch=1`(터치 UI 강제) / `xr=1`(실험적 WebXR: VR 버튼 표시, 왼스틱 이동·오른스틱 45° 스냅 회전, 헤드셋에서 미검증) / `walk=1&x=&z=&y=`(y를 주면 그 높이 근처의 데크(다리·탑 층)에 올려 놓음, 예 `walk=1&x=-12.6&z=-0.7&y=120.5&yaw=-49` = 탑 2층).
...
- 에펠탑: 내부 투광기처럼 아랫면·안쪽 면이 밝은 금색, 일몰 후 매시 정각 5분간 흰 반짝임(6000개 전구), 23:45 소등 후 비콘만. `?tower=lit`로 밤새 켤 수 있습니다. 꼭대기 **등대**(1999년 12월 31일 점등, 6000 W 제논 해상용 투광기 4기를 동기화한 양방향 교차 빔, 도달 거리 80 km)가 조명과 함께 켜져 40초에 한 바퀴(영상에서 추정) 하늘을 쓸고 지나갑니다.
...
- **조각·기념물 메시**(`scripts/config.ts`의 `STATUES` → 런타임 `Furniture`): 자유의 불꽃(도금 횃불), **평화의 벽**(Clara Halter · Jean-Michel Wilmotte, 2000–2020: 16.4 × 13.8 × 9 m, 나무 단 위의 강철 골조 캐노피와 '평화'를 49개 언어로 새긴 유리 벽 두 장, 양 옆에 축과 나란히 2열 × 8개씩 선 스테인리스 기둥 32개(약 7 m)), 그 밖의 OSM 조각상은 익명 인물상.
