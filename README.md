...
빛·물: `lamps=0`(가로등 광원 끄기) / `lampsdebug=1`(광원 마젠타) / `refl=0|1|2`(반사 끄기·절반·1/4 해상도) / `stars=0` / `glow=0`(광공해 끄기) / `headlights=0` / `shoplights=0` / `signals=0`(신호등 끄기) / `wet=1`(비 온 뒤 노면) / `tower=lit`(탑 조명 밤새 유지) / `ao=0`(AO 끄기).
...
UX: `minimap=1`(미니맵 켠 채 시작) / `audio=0|debug` / `touch=1`(터치 UI 강제) / `walk=1&x=&z=&y=`(y를 주면 그 높이 근처의 데크(다리·탑 층)에 올려 놓음, 예 `walk=1&x=-12.6&z=-0.7&y=120.5&yaw=-49` = 탑 2층).
...
```bash
npm test          # 좌표 변환, 압출 지오메트리 방향 테스트
