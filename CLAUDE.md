...
- UI: `index.html` holds the DOM and CSS in fixed HUD zones (top-left landmark card, top-centre compass + streaming counter, top-right clock chip + time panel, bottom-left hint bar + diagnostics, bottom-centre prompt/toast, bottom-right minimap, centred `.modal` panels). `#overlay` is the start screen until the first start and the translucent pause menu afterwards (`Hud.markStarted`). One modal at a time through `App.openModal/closeModal` (releases / re-grabs the pointer). User settings live in `shared/prefs.ts` (pure, tested) + `src/ui/Prefs.ts` (localStorage `paris.prefs`) and are pushed by `App.applyPrefs`; URL parameters are applied afterwards so they win for that session without being saved. Diagnostics (stats-gl + status line) are off unless `?status=1`, `H` twice or the setting. `window.app` is exposed for console / screenshot automation.
